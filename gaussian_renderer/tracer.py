...
try:
    ...
    _src_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "gaussian_tracer")
    ...
    # os.system(f"cd {_src_path} && mkdir build && cd build && cmake .. && make")
...
class _GaussianTrace(torch.autograd.Function):
    ...
    def backward(ctx, grad_out_color, grad_out_depth, grad_out_alpha):
        ...
        # import pdb;pdb.set_trace()
        # m=trimesh.Trimesh(means3D.detach().cpu().numpy()).export("test.ply")
        # mask = grad_shs.norm(dim=-1).norm(dim=-1) > 0
        # m=trimesh.Trimesh(means3D[mask].detach().cpu().numpy()).export("test2.ply")
        # save_image(colors.reshape(800,800,3).permute(2, 0, 1), "test2.png")
        # save_image(colors2.reshape(800,800,3).permute(2, 0, 1), "test3.png")
        # save_image((grad_out_color!=0).float().reshape(800,800,3).permute(2, 0, 1), "test4.png")
        # mask = grad_w > 0
        # m=trimesh.Trimesh(means3D[mask].detach().cpu().numpy()).export("test3.ply")
        # grad_out_color
        # save_image(grad_out_color.abs().reshape(800,800,3).permute(2, 0, 1), "test4.png")
        # mask = grad_out_color.norm(dim=-1)>0
        # grad_out_color[mask].abs()
