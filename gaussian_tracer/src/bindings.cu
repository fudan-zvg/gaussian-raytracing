...
public:
    ...
    void build_bvh(const torch::Tensor& triangles){
        ...
        triangle_bvh->build_bvh((Triangle*)triangles.data_ptr<float>(), n_triangles, m_stream);
    }

    void update_bvh(const torch::Tensor& triangles){
        const size_t n_triangles = triangles.size(0);
        cudaStream_t m_stream = at::cuda::getCurrentCUDAStream();;
        triangle_bvh->update_bvh((Triangle*)triangles.data_ptr<float>(), n_triangles, m_stream);
    }
    ...
    void trace_forward(
        ...
        triangle_bvh->gaussian_trace_forward(
            n_elements, (const vec3*)rays_o.data_ptr<float>(), (const vec3*)rays_d.data_ptr<float>(), gs_idxs.data_ptr<int>(), 
            (const vec3*)means3D.data_ptr<float>(), opacity.data_ptr<float>(), (const mat3x3*)SinvR.data_ptr<float>(), (const vec3*)shs.data_ptr<float>(), 
            (vec3*)colors.data_ptr<float>(), depth.data_ptr<float>(), alpha.data_ptr<float>(), 
    ...
    void trace_backward(
        ...
        const torch::Tensor grad_out_color, const torch::Tensor grad_out_depth, const torch::Tensor grad_out_alpha, torch::Tensor colors2, torch::Tensor grad_w, 
        const float alpha_min, const float transmittance_min, const int deg
        ){
        const uint32_t n_elements = rays_o.size(0);
        cudaStream_t stream = at::cuda::getCurrentCUDAStream();
        ...
        triangle_bvh->gaussian_trace_backward(
            n_elements, (const vec3*)rays_o.data_ptr<float>(), (const vec3*)rays_d.data_ptr<float>(), gs_idxs.data_ptr<int>(), 
            (const vec3*)means3D.data_ptr<float>(), opacity.data_ptr<float>(), (const mat3x3*)SinvR.data_ptr<float>(), (const vec3*)shs.data_ptr<float>(), 
            (const vec3*)colors.data_ptr<float>(), depth.data_ptr<float>(), alpha.data_ptr<float>(), 
            (vec3*)grad_means3D.data_ptr<float>(), grad_opacity.data_ptr<float>(), (mat3x3*)grad_SinvR.data_ptr<float>(), (vec3*)grad_shs.data_ptr<float>(), 
            (const vec3*)grad_out_color.data_ptr<float>(), grad_out_depth.data_ptr<float>(), grad_out_alpha.data_ptr<float>(), (vec3*)colors2.data_ptr<float>(), grad_w.data_ptr<float>(),
            alpha_min, transmittance_min, deg, max_coeffs, stream);
    }
